...
double seasonal_transmission_factor( double t, prms* ppc );
...
int func(double t, const double y[], double f[], void *params)
{
    ...
    double trr = ((double)NUMR) / ppc->v[i_immune_duration];
    double stf = ppc->seasonal_transmission_factor(t);
        ...
        for(vir=0; vir<NUMSEROTYPES; vir++)
        {
            for(stg=0; stg<NUMR; stg++)
            {
                ...
                if( stg==0 )
                {
                    ...
                    f[ NUMSEROTYPES*NUMR*loc + NUMR*vir + stg ] += ppc->v[i_nu] * y[ STARTI + NUMSEROTYPES*loc + vir ];    
                }
                else
                {
                ...
                for(int l=0; l<NUMLOC; l++)
                    for(int v=0; v<NUMSEROTYPES; v++)
                        sum_foi += ppc->sigma[vir][v] * ppc->beta[v] * stf /*ppc->seasonal_transmission_factor(t)*/ * ppc->eta[loc][l] * y[ STARTI + NUMSEROTYPES*l + v ];
    ...
    for(loc=0; loc<NUMLOC; loc++)
    {
        for(vir=0; vir<NUMSEROTYPES; vir++)
        {
            ...
            for(int l=0; l<NUMLOC; l++) 
                foi_on_susc_single_virus += ppc->eta[loc][l] * stf /*ppc->seasonal_transmission_factor(t)*/ * ppc->beta[vir] * y[ STARTI + NUMSEROTYPES*l + vir ];
            ...
            for(int l=0; l<NUMLOC; l++)             // sum over locations
                for(int v=0; v<NUMSEROTYPES; v++)   // sum over recent immunity
                    for(int s=0; s<NUMR; s++)       // sum over R stage
                        inflow_from_recovereds += ppc->sigma[vir][v] * stf /*ppc->seasonal_transmission_factor(t)*/ * ppc->beta[vir] * ppc->eta[loc][l] * y[ STARTI + NUMSEROTYPES*l + vir ] * y[ NUMSEROTYPES*NUMR*loc + NUMR*v + s ];
            ...
            f[ STARTI + NUMSEROTYPES*loc + vir ] += - ppc->v[i_nu] * y[ STARTI + NUMSEROTYPES*loc + vir ];
    ...
    for(loc=0; loc<NUMLOC; loc++)
    {
        ...
        for(int l=0; l<NUMLOC; l++)
            for(int v=0; v<NUMSEROTYPES; v++) 
                foi_on_susc_all_viruses += ppc->eta[loc][l] * stf /*ppc->seasonal_transmission_factor(t)*/ * ppc->beta[v] * y[ STARTI + NUMSEROTYPES*l + v ];
