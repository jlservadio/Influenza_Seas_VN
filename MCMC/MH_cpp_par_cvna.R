...
load('~/work/Infl_VN/Data/CVN_Detr_Ag.Rdata')
dat = dat.cent
...
all_params = c(
	...
	'Na' = 0.8 * 1e6, 
	'Nb' = 0.2 * 1e6, 
	'beta_H1' = 0.1875, #runif(1, 0.19, 0.23), # 0.26, 
	'beta_B' = 0, # 0.23, 
	'beta_H3' = 0, 
	'nu_denom' = 5, 
	'amp' = 0.20, 
	'rho_denom' = 1200, #sample(c(400:1200), 1), 
	'sigma12' = 0.75, 
	'sigma13' = 0.50, 
	'sigma23' = 0.75, 
	'etaab' = 0.01, 
	'etaba' = 0.05, 
	'phi.1' = 163, #sample(c(1:400), 1), # 250, 
	'tau.1' = 369, #sample(c(100:650), 1), # 500, 
	'tau.2' = 325, #sample(c(100:650), 1), # 500, 
	'tau.3' = 172, #sample(c(100:650), 1), # 300, 
	'tau.4' = 282, #sample(c(100:650), 1), # 400, 
	'tau.5' = 330, #sample(c(100:650), 1), # 200, 
	'tau.6' = 196, #sample(c(100:650), 1), # 400, 
	'tau.7' = 240, #sample(c(100:650), 1), # 400, 
	'tau.8' = 230, #sample(c(100:650), 1), # 350, 
	'tau.9' = 350, #sample(c(100:650), 1), # 350, 
	'tau.10' = 222, #sample(c(100:650), 1), # 350, 
	'mu.tau' = 265, 
	'sigma.tau' = 100, 
	'report' = 0.08, 
	'norm_var' = 0.1^2, 
	'epidur' = 80, #sample(c(30:150), 1), 
	'zeroprevlevel' = 3.4, #sample(c(0:500), 1),
...
if (file.num > 1) {
	...
	load(paste('~/work/Infl_VN/MCMC_0501/MCMC_cvna/MCMC_cvna_0501_', file.num-1, '.Rdata', sep = ''))
	...
	# load('~/work/Infl_VN/MCMC_0501/MCMC_cvna/MCMC_cvna_0509.Rdata')
	load('~/work/Infl_VN/MCMC_0501/MCMC_cvna/MCMC_cvna_0501_0.Rdata')
...
file.name = paste('~/work/Infl_VN/MCMC_0501/MCMC_cvna/MCMC_cvna_0501_', file.num, '.Rdata', sep = '')

search_params_names = c('beta_H1', 'amp', 'rho_denom', 
   	'phi.1', 'tau.1', 'tau.2', 'tau.3', 'tau.4', 'tau.5', 'tau.6', 'tau.7', 'tau.8', 'tau.9', 'tau.10', 
...
mv.pars = list(c('beta_H1'), c('amp'), c('rho_denom'), c('phi.1'), 
	c('tau.1', 'tau.2', 'tau.3', 'tau.4', 'tau.5', 'tau.6', 'tau.7','tau.8', 'tau.9', 'tau.10'), 
...
pp = MH_chain(ch.num = 0, chain.length = 200, 
	...
	use.cur = use.cur,
	type = 'cvna', 
	dat = as.matrix(cbind(dat$ILI_p_A, rep(NA, nrow(dat)), rep(NA, nrow(dat)))))
...
if (file.num >= 3) { 
	...
	all.data = list()
	load('~/work/Infl_VN/MCMC_0501/MCMC_cvna/MCMC_cvna_0501_1.Rdata')
	...
	for (i in 2:file.num) {
		...
		load(paste('~/work/Infl_VN/MCMC_0501/MCMC_cvna/MCMC_cvna_0501_', i, '.Rdata', sep = ''))
	...
	save(all.data, file = paste('~/work/Infl_VN/MCMC_0501/MCMC_cvna/MCMC_cvna_0501.Rdata', sep = ''))
...
type = 'cvna'; source('~/work/Infl_VN/MCMC_0501/Code/plotting.R')
