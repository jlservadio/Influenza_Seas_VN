...
all_params = c(
	...
	'Na' = 0.8 * 1e6, 
	'Nb' = 0.2 * 1e6, 
	'beta_H1' = 0.196, #runif(1, 0.19, 0.23), # 0.26, 
	'beta_B' = 0, # 0.23, 
	'beta_H3' = 0, 
	'nu_denom' = 5, 
	'amp' = 0.335, #0.07, 
	...
	'etaab' = 0.01, 
	'etaba' = 0.05, 
	'phi.1' = 40, #sample(c(1:400), 1), # 250, 
	'tau.1' = 100, #sample(c(100:650), 1), # 500, 
	'tau.2' = 405, #sample(c(100:650), 1), # 500, 
	'tau.3' = 333, #sample(c(100:650), 1), # 300, 
	'tau.4' = 295, #sample(c(100:650), 1), # 400, 
	'tau.5' = 161, #sample(c(100:650), 1), # 200, 
	'tau.6' = 315, #sample(c(100:650), 1), # 400, 
	'tau.7' = 160, #sample(c(100:650), 1), # 400, 
	'tau.8' = 390, #sample(c(100:650), 1), # 350, 
	'tau.9' = 108, #sample(c(100:650), 1), # 350, 
	'tau.10' = 276, #sample(c(100:650), 1), # 350, 
	'tau.11' = 232, #sample(c(100:650), 1), # 350, 
	'tau.12' = 475, #sample(c(100:650), 1), # 350, 
	'mu.tau' = 465, 
	'sigma.tau' = 100, 
	'report' = 0.022, #0.00026, 
	'norm_var' = 0.1^2, 
	'epidur' = sample(c(30:150), 1), 
	'zeroprevlevel' = 20, #sample(c(0:500), 1),
	'immig' = 78 #sample(seq(10, 500, by = 10), 1)
	)
...
if (file.num > 1) {
	...
	load(paste('~/work/Infl_VN/MCMC_0501/MCMC_nvnc/MCMC_nvnc_0501_', file.num-1, '.Rdata', sep = ''))
	...
	# load('~/work/Infl_VN/MCMC_0501/MCMC_nvnc/MCMC_nvnc_0509.Rdata')
	load('~/work/Infl_VN/MCMC_0501/MCMC_nvnc/MCMC_nvnc_0501_0.Rdata')

	start.pars = list()
	for (i in 1:4) {
		ss = sample(c(1:2), 1); ss = i
		start.pars[[i]] = pps[[ss]][which(pps[[ss]][ , 'post'] == max(pps[[ss]][ , 'post']))[1], names(all_params)]

		# start.pars[[i]]['norm_var'] = all_params['norm_var']
	}
...
file.name = paste('~/work/Infl_VN/MCMC_0501/MCMC_nvnc/MCMC_nvnc_0501_', file.num, '.Rdata', sep = '')

search_params_names = c('beta_H1', 'amp', 'rho_denom', 
   	'phi.1', 'tau.1', 'tau.2', 'tau.3', 'tau.4', 'tau.5', 'tau.6', 'tau.7', 'tau.8', 'tau.9',
	'tau.10', 'tau.11', 'tau.12',  
...
if (file.num >= 3) { 
	...
	all.data = list()
	load('~/work/Infl_VN/MCMC_0501/MCMC_nvnc/MCMC_nvnc_0501_1.Rdata')
	...
	for (i in 2:file.num) {
		...
		load(paste('~/work/Infl_VN/MCMC_0501/MCMC_nvnc/MCMC_nvnc_0501_', i, '.Rdata', sep = ''))
	...
	save(all.data, file = paste('~/work/Infl_VN/MCMC_0501/MCMC_nvnc/MCMC_nvnc_0501.Rdata', sep = ''))
...
type = 'nvnc'; source('~/work/Infl_VN/MCMC_0501/Code/plotting.R')
