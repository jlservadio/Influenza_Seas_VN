...
dat.0 = read.csv('~/work/Infl_VN/Data/ECDC_Flu_Cleaned.csv')
dat = dat.0[dat.0$RegionCode == 'DK', ]
dat = dat[dat$Year %in% c(2010:2019) & dat$Week <= 52, ]

if (dat$RegionCode[1] == 'DK') {
	add = dat[c(1:2), ]
	add$Week = 39; add$Year = c(2015, 2016)
	add$Pos_Sentinel = 0; add$Pos_Nonsentinel = 0; add$Positivity_Sentinel = 0
	dat = rbind(dat, add)
	dat = dat[order(dat$Year, dat$Week), ]
}

dat$Time = dat$Year + ((dat$Week-1)/52)
dat$ILI_p_H10 = dat$Pos_Sentinel + dat$Pos_Nonsentinel

dat$ILI_p_H1 = 0; dat$ILI_p_B = 0; dat$ILI_p_H3 = 0

for (i in 1:nrow(dat)) {

	dat$ILI_p_H1[i] = dat$ILI_p_H10[i] / 
		mean(dat$ILI_p_H10[which(abs(dat$Time - dat$Time[i]) <= 0.5)], na.rm = TRUE)

}

dat$ILI_p = dat$ILI_p_H1
...
all_params = c(
	...
	'Na' = 0.8 * 1e6, 
	'Nb' = 0.2 * 1e6, 
	'beta_H1' = runif(1, 0.19, 0.23), # 0.26, 
	'beta_B' = 0, # 0.23, 
	'beta_H3' = 0, 
	'nu_denom' = 5, 
	'amp' = 0.07, 
	'rho_denom' = sample(c(400:1200), 1), 
	'sigma12' = 0.75, 
	'sigma13' = 0.50, 
	'sigma23' = 0.75, 
	'etaab' = 0.01, 
	'etaba' = 0.05, 
	'phi.1' = sample(c(1:400), 1), # 250, 
	'tau.1' = sample(c(100:650), 1), # 500, 
	'tau.2' = sample(c(100:650), 1), # 500, 
	'tau.3' = sample(c(100:650), 1), # 300, 
	'tau.4' = sample(c(100:650), 1), # 400, 
	'tau.5' = sample(c(100:650), 1), # 200, 
	'tau.6' = sample(c(100:650), 1), # 400, 
	'tau.7' = sample(c(100:650), 1), # 400, 
	'tau.8' = sample(c(100:650), 1), # 350, 
	'tau.9' = sample(c(100:650), 1), # 350, 
	'mu.tau' = 465, 
	'sigma.tau' = 100, 
	'report' = 0.00026, 
	'norm_var' = 0.1^2, 
	'epidur' = sample(c(30:150), 1), 
	'zeroprevlevel' = sample(c(0:500), 1),
	'immig' = sample(seq(10, 500, by = 10), 1)
	)
...
if (file.num > 1) {
	...
	load(paste('~/work/Infl_VN/MCMC_0501/MCMC_dk/MCMC_dk_0501_', file.num-1, '.Rdata', sep = ''))
	...
	load('~/work/Infl_VN/MCMC_0501/MCMC_dk/MCMC_dk_0501_0.Rdata')
...
file.name = paste('~/work/Infl_VN/MCMC_0501/MCMC_dk/MCMC_dk_0501_', file.num, '.Rdata', sep = '')
...
pps = mclapply(1:num.chains, function(x) {
	...
	pp = MH_chain(ch.num = x, chain.length = chain.length, 
		...
		all_params_sigma = all_params_sigma*0.5, 
		all_params_bounds = list(all_params_leftbound, all_params_rightbound), 
		priors_fams = prior_family, 
		priors_pars = prior_params, 
		mv.pars = mv.pars, 
...
if (file.num >= 3) { 
	...
	all.data = list()
	load('~/work/Infl_VN/MCMC_0501/MCMC_dk/MCMC_dk_0501_1.Rdata')
	...
	for (i in 2:file.num) {
		...
		load(paste('~/work/Infl_VN/MCMC_0501/MCMC_dk/MCMC_dk_0501_', i, '.Rdata', sep = ''))
	...
	save(all.data, file = paste('~/work/Infl_VN/MCMC_0501/MCMC_dk/MCMC_dk_0501.Rdata', sep = ''))
...
type = 'dk'; source('~/work/Infl_VN/MCMC_0501/Code/plotting.R')
